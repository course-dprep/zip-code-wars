...
download.file(
  url,
  destfile = "yelp_academic_dataset_business.csv",
  mode = "wb"
)

yelp_business <- read.csv("yelp_academic_dataset_business.csv")

# Explore postal code variable
summary(yelp_business$postal_code)

# Extract columns needed for analysis
columns <- c("state","postal_code","city","review_count","name","categories","stars","business_id")
...
#Convert json into csv (do this in terminal)
cd ~/Desktop/yelp_convert 
python3 --version
python3 -m pip install pandas 
python3 json_to_csv_converter.py yelp_academic_dataset_checkin.json

# Load datset checkin
library(readr)
yelp_checkin <- read_csv("Convert/yelp_academic_dataset_checkin.csv")
View(yelp_checkin)

#Load dataset user
library(readr)
yelp_user <- read_csv("Convert/yelp_academic_dataset_user.csv")
View(yelp_user)
...
#Calculate total of checkin
yelp_checkin <- yelp_checkin %>%
  mutate(
    total_checkins = ifelse(
      is.na(date) | date == "",
      0,
      str_count(date, ",") + 1
    )
...
#Merge dataset 
merged_data <- research_project_filtered %>%
  left_join(
    yelp_checkin %>% select(business_id, total_checkins),
    by = "business_id"
  )

#Use as control variable 
review_count is a biased proxy for popularity because not all visitors leave reviews and review 
propensity differs by industry 

# Creating log-scaled density variable (log1p is used to better handle 0 values in density)
merged_data <- research_project_density %>%
  mutate(
    log_density = log1p(industry_zip_count))
merged_data2$total_checkins <- as.numeric(merged_data$total_checkins)
merged_data2$log_checkins <- log1p(merged_data$total_checkins)

# Base Regression model tested (without interaction effect)
regression <- lm(stars ~ log_density + review_count,
                      data = merged_data)
summary(regression)
...
though the effect size per review is very small. However, the overall explanatory power of the model is very low, 
with an R squared of 0.004, indicating that density and review volume explain only a small fraction of the variation 
in ratings.
