...
This study uses Dataset 1: Yelp Business Data from the publicly available Yelp Academic Dataset. The dataset contains structured business level information including business name, address, city, state, postal code, categories, review count, and average star rating. The dataset was accessed via Google Drive by copying the shared file link in the format file/d/FILE_ID/ and extracting the unique file identifier. The file was then imported into the R environment using the googledrive package. To avoid repeated authentication prompts during the session, authentication was handled programmatically using drive_auth() and drive_deauth() where appropriate. After cleaning the dataset, the final sample consists of 150273 business observations.

_Variable operationalisation_
